...
class PbapClientConnectionHandler extends Handler {
    ...
    static final int MSG_DOWNLOAD_PHONEBOOK = 4;
    static final int MSG_DOWNLOAD_CALL_LOG = 5;
    ...
    PbapClientConnectionHandler(Looper looper, Context context, PbapClientStateMachine stateMachine,
        ...
        mAccount = new Account(mDevice.getAddress(), mContext.getString(R.string.pbap_account_type));
    }
    ...
    PbapClientConnectionHandler(Builder pceHandlerbuild) {
        ...
        onPhonebookPullStateChangeListener = pceHandlerbuild.onPhonebookPullStateChangeListener;
        onCallLogPullStateChangeListener = pceHandlerbuild.onCallLogPullStateChangeListener;
    ...
    public static class Builder {
        ...
        public Builder setOnPhonebookPullStateChangeListener(
        ...
        public Builder setOnCallLogPullStateChangeListener(
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MSG_CONNECT:
                boolean connectSocket = connectSocket();
            ...
            case MSG_DOWNLOAD:
                ...
                phonebookPulling = true;
                callLogPulling = true;
                // Start at contact 1 to exclued Owner Card PBAP 1.1 sec 3.1.5.2
                if (onPhonebookPullStateChangeListener != null) {
                    onPhonebookPullStateChangeListener.onPhonebookPullStart(mDevice);
                }
                if (onCallLogPullStateChangeListener != null) {
                    onCallLogPullStateChangeListener.onCallLogPullStart(mDevice);
                }
                pullPhonebook();
                pullCallLog();
                break;
            case MSG_DOWNLOAD_PHONEBOOK:
                if (onPhonebookPullStateChangeListener != null) {
                    onPhonebookPullStateChangeListener.onPhonebookPullStart(mDevice);
                }
                pullPhonebook();
                break;
            case MSG_DOWNLOAD_CALL_LOG:
                if (onCallLogPullStateChangeListener != null) {
                    onCallLogPullStateChangeListener.onCallLogPullStart(mDevice);
                }
                pullCallLog();
    ...
    private boolean phonebookPulling = false;
    private boolean callLogPulling = false;
    private ArrayList<BluetoothPhonebook> phoneBookList = new ArrayList<>();
    private ArrayList<BluetoothCallLog> callLogList = new ArrayList<>();

    public boolean isPhonebookPulling() {
        return phonebookPulling;
    }

    public boolean isCallLogPulling() {
        return callLogPulling;
    }
    ...
    private void pullPhonebook() {
        try {
            ...
            phoneBookList.clear();
            phoneBookList.addAll(processor.getPhoneBookList());
            if (onPhonebookPullStateChangeListener != null) {
                onPhonebookPullStateChangeListener.onPhonebookPullComplete(mDevice, new ArrayList<>(phoneBookList));
            }
            phonebookPulling = false;
        } catch (IOException e) {
            phonebookPulling = false;
    ...
    private void pullCallLog() {
        try {
            callLogPulling = true;
            HashMap<String, Integer> callCounter = new HashMap<>();
            ArrayList<BluetoothCallLog> recordList = new ArrayList<>();
            downloadCallLog(MCH_PATH, callCounter, recordList);
            downloadCallLog(ICH_PATH, callCounter, recordList);
            downloadCallLog(OCH_PATH, callCounter, recordList);
            Collections.sort(recordList, (record1, record2) ->
                    Long.compare(record2.getTime(), record1.getTime()));
            if (onCallLogPullStateChangeListener != null) {
                onCallLogPullStateChangeListener.onCallLogPullComplete(mDevice, recordList);
            }
            callLogList.clear();
            callLogList.addAll(recordList);
            callLogPulling = false;
        } catch (Exception e) {
            callLogPulling = false;
            Log.w(TAG, "DOWNLOAD_CONTACTS Failure" + e.toString());
        }
    ...
    private boolean connectSocket() {
        try {
            ...
            if (mPseRec == null) {
                ...
                mSocket = mDevice.createRfcommSocketToServiceRecord(BluetoothUuid.PBAP_PSE.getUuid());
    ...
    void downloadCallLog(String path, HashMap<String, Integer> callCounter, ArrayList<BluetoothCallLog> recordList) {
        try {
            BluetoothPbapRequestPullPhoneBook request =
            ...
            CallLogPullRequest processor =
                    new CallLogPullRequest(mPbapClientStateMachine.getContext(), path,
                            callCounter, mAccount, recordList);
            processor.setResults(request.getList());
            processor.onPullComplete();
        } catch (IOException e) {
