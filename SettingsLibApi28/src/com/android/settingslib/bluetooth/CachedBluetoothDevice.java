...
public class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    private void connectAutoConnectableProfiles() {
        ...
        // Reset the only-show-one-error-dialog tracking variable
        mIsConnectingErrorPossible = true;
        Log.e("wtf", "mProfiles = " + mProfiles.size());
        for (LocalBluetoothProfile profile : mProfiles) {
            Log.e("wtf", "");
    ...
    public void unpair() {
        ...
        if (state != BluetoothDevice.BOND_NONE) {
            ...
            boolean hasIsTwsPlusDeviceMethod = false;
            Method[] methods = BluetoothDevice.class.getDeclaredMethods();
            for (Method method : methods) {
                if ("isTwsPlusDevice".equals(method.getName())) {
                    hasIsTwsPlusDeviceMethod = true;
                    break;
                }
            }
            if (hasIsTwsPlusDeviceMethod) {
                if (mDevice.isTwsPlusDevice()) {
    ...
    public boolean getA2dpOrHfpHasConnected() {
